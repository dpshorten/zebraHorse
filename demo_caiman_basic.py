...
# %% set up some parameters
    ...
    fnames = [os.path.join(caiman_datadir(), 'FirstDay', 'test011-ch1.tif')]
                            # file(s) to be analyzed
    is_patches = False       # flag for processing in patches or not
    fr = 2                  # approximate frame rate of data
...
# %% COMPONENT EVALUATION
    ...
    object_methods = [method_name for method_name in dir(cnm2.estimates)
                  if callable(getattr(cnm2.estimates, method_name))]

    print(object_methods)
    cnm2.estimates.detrend_df_f()
    print(cnm2.estimates.F_dff)
    import pickle
    f = open("../zebraHorse/df_f.pkl", "wb")
